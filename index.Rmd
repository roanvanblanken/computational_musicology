...
ui <- dashboardPage(
  dashboardHeader(title = "Spotify Playlist Dashboard"),
  dashboardSidebar(
    sidebarMenu(
...
# Define server for dashboard
server <- function(input, output) {
  # Get playlist information from Spotify API
  playlist_data <- get_playlist_tracks("6pl0C7qbIl5uoY3Tdf82oa", market = "US")
  # Extract track information from playlist data
  playlist_table <- data.frame(
    track_name = unlist(lapply(playlist_data$items, function(x) x$track$name)),
    artist_name = unlist(lapply(playlist_data$items, function(x) x$track$artists[[1]]$name)),
    album_name = unlist(lapply(playlist_data$items, function(x) x$track$album$name)),
    release_date = unlist(lapply(playlist_data$items, function(x) x$track$album$release_date)),
    stringsAsFactors = FALSE
  )
  # Display playlist table
  output$playlist_table <- renderTable(playlist_table)
}
