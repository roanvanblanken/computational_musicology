...
Track-Level Summary {data-icon="fa-chart-line"}
=====================================
...
songs_i_dislike <- get_playlist_audio_features("", "4bJQX5w7W4wEnHLmWqUIVY") %>%
  mutate(playlist = 'Songs I dislike')

# Combine the playlists into one data frame
df <- rbind(songs_i_like, songs_i_dislike)

# Remove non-plotable features (e.g., id, uri, etc.)
df <- df[,c(2, 6:11, 13:16)]

# Calculate the mean for each feature by playlist
df_means <- df %>%
  group_by(playlist) %>%
  summarise(across(danceability:tempo, mean))

# Create a data frame for plotting
plot_data <- rbind(df_means, df)
...
radarchart(plot_data, 
           ...
           plwd=2, 
           pty=20, 
           title="My Music Preferences by Playlist", 
           seg=6, 
           maxmin=TRUE, 
           vlabels=variables, 
           vlcex=0.8, 
